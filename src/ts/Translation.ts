    ...
    switch (options.translatorId) {
      case Translators.CHUTES_TRANSLATE:
        this.translateText = async (resolve) => {
          ...
          const searchResults = await this.webSearchWithTavily(this.text, TVLY_API_KEY as string, this.abortController.signal).then(value => value.map((element, index) => `[webpage ${index + 1} begin]${element}[webpage ${index + 1} end]`).join('\n'))
          const date = new Date()
