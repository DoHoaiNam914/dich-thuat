...
class Translation {
    ...
    switch (options.translatorId) {
      case Translators.CHUTES_TRANSLATE:
        this.translateText = async (resolve) => {
          ...
          const searchResults = isChutesWebSearchEnabled ? await this.webSearchWithTavily(this.text, TVLY_API_KEY as string, this.abortController.signal).then(value => value.map((element, index) => `[webpage ${index + 1} begin]${element}[webpage ${index + 1} end]`).join('\n')) : ''
          const date = new Date()
      ...
      case Translators.GROQ_TRANSLATE: {
        ...
        this.translateText = async (resolve) => {
          ...
          const searchResults = isGroqWebSearchEnabled ? await this.webSearchWithTavily(this.text, TVLY_API_KEY as string, this.abortController.signal).then(value => value.map((element, index) => `[webpage ${index + 1} begin]${element}[webpage ${index + 1} end]`).join('\n')) : ''
          const date = new Date()
          const dateTimeFormat = new Intl.DateTimeFormat(isNotDeepseekModel ? 'en' : 'zh-CN', {
            day: 'numeric',
            month: 'long',
  ...
  private async webSearchWithTavily (query: string, apiKey: string, signal = new AbortSignal()): Promise<string[]> {
  ...
  private getSystemInstructions (systemInstruction: SystemInstructions, text: string, originalLanguage: string | null, destinationLanguage: string, options: Options): string[] {
    ...
    switch (systemInstruction) {
      ...
      case SystemInstructions.DOCTRANSLATE_IO: {
        ...
        const domain: string = (options.domain as Domains).replace(Domains.NONE, 'Lifestyle')
        ...
        const tone: string = (options.tone as Tones).replace(Tones.NONE, Tones.SERIOUS)
