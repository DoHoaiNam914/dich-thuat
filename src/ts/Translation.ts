...
export default class Translation {
        ...
        this.translateText = async (resolve = (translatedText: string, text: string, options) => console.log(`${JSON.stringify(options)}\n### TEXT SENTENCE:\n${text}\n### TRANSLATED TEXT:\n${translatedText}`)) => {
          ...
          await window.fetch('https://gateway.api.airapps.co/aa_service=server5/aa_apikey=5N3NR9SDGLS7VLUWSEN9J30P//v3/proxy/open-ai/v1/responses', {
            body: JSON.stringify({
              model: openaiModelId,
              input: [
                ...this.getSystemInstructions(systemInstruction, destinationLanguage, originalLanguage, options).map(element => ({
      ...
      default: {
        this.translateText = async (resolve = (translatedText: string, text: string, options) => console.log(`${JSON.stringify(options)}\n### TEXT SENTENCE:\n${text}\n### TRANSLATED TEXT:\n${translatedText}`)) => {
          ...
          const config = {
            ...
            systemInstruction: this.getSystemInstructions(systemInstruction, destinationLanguage, originalLanguage, options).map(element => ({
              text: element
            }))
          }
  ...
  private getSystemInstructions (systemInstruction, destinationLanguage, originalLanguage, options): string[] {
