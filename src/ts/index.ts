...
$( document ).ready(() => {
  ...
  $checkedOptions.each((_index, element) => {
    $( element ).prop( "checked", Boolean(sessionStorage.getItem(($( element ).prop( "id" ) as string).split('-').slice(0, -1).map((element, index) => index > 0 ? element.charAt(0).toUpperCase() + element.substring(1) : element).join(''))) ?? $( element ).prop( "checked" ) )
  })
