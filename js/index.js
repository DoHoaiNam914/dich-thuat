  ...
  Papa.parse($(this).prop('files')[0], {
    ...
    complete: (results) => {
      customDictionary = results.data.map(a => {
        ...
        const row = { ori_lang: '', des_lang: '', ori_word: '', des_word: '' }
...
$('[data-define-url]').on('click', function () {
  ...
  const url = `${method.endsWith('WithCorsProxy') ? Utils.CORS_HEADER_PROXY : ''}${$(this).data('define-url').replace('%l', $sourceTextLanguageSelect.val().split('-')[0]).replace('%s', sourceText)}`
  switch (method) {
