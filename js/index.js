...
$(document).ready(() => {
  ...
  $('.string-value-option').each((_index, element) => {
    $(element).val(sessionStorage.getItem($(element).prop('id').split('-').slice(0, -1).map((element, index) => index > 0 ? element.charAt(0).toUpperCase() + element.substring(1) : element).join('')) ?? $(element).val())
  })
  ...
  $('.checked-option').each((_index, element) => {
    $(element).prop('checked', sessionStorage.getItem($(element).prop('id').split('-').slice(0, -1).map((element, index) => index > 0 ? element.charAt(0).toUpperCase() + element.substring(1) : element).join('')) ?? $(element).prop('checked'))
  })
...
$('[data-define-url]').on('click', function () {
  if ($sourceText.val().length === 0) { return }
...
$('.language-select').on('change', function () {
  ...
  $textLanguageSelect.val($textLanguageSelect.prop('disabled') !== false && value !== 'null' ? value : $textLanguageSelect.val()).change()
})
  ...
  switch ($(this).text()) {
    case 'Dịch': {
      ...
      textareaTranslation.translateText(appendTranslatedTextIntoOutputTextarea).then(() => {
        ...
        if (!textareaTranslation.abortController.signal.aborted) { $(this).click() }
      })
      break
    }
    case 'Huỷ':
